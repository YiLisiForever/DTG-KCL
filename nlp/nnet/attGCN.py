...
class AttGcnLSTM(nn.Module):
    ...
    def __init__(self, config, usegpu):
        super(AttGcnLSTM, self).__init__()
        self.usegpu = usegpu
        ...
        """
        self.gcn_attention_1 = AttentionTanH(config)
        self.gcn_attention_2 = AttentionTanH(config)
        """
        self.gcn_attention_1 = nn.MultiheadAttention(features, num_heads=8, batch_first=True)
        self.gcn_attention_2 = nn.MultiheadAttention(features, num_heads=8, batch_first=True)
...
# ----------------------第一层任务卷积-------------------------*
        ...
        for a in range(1, len(task_name) + 1):
            ...
            stacked = torch.stack(neig_rep_list, dim=1)  # [batch, len(list), hidden]
            # -------------try多头注意力-------------
            Q_squ = self.hidden_list[a].unsqueeze(1)  # (32, 1, 512)
            ronghe_rep,_ = self.gcn_attention_1(Q_squ, stacked, stacked)
            ronghe_rep = ronghe_rep.squeeze(1)
            # -------------try多头注意力-------------
            h_conv = torch.tanh(self.Full_inter_2(self.hidden_list[a] + ronghe_rep))
...
# ----------------------第二层任务卷积-------------------------*
        ...
        for a in range(1, len(task_name) + 1):
            ...
            stacked = torch.stack(neig_rep_list, dim=1)  # [batch, len(list), hidden]
            # -------------try多头注意力-------------
            Q_squ = new_hn[a].unsqueeze(1)  # (32, 1, 512)
            ronghe_rep,_ = self.gcn_attention_2(Q_squ, stacked, stacked)
            ronghe_rep = ronghe_rep.squeeze(1)
            # -------------try多头注意力-------------
            h_conv = torch.tanh(self.Full_inter_2(new_hn[a] + ronghe_rep))
