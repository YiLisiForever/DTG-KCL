...
import pdb
import time
from data.deve_formatter import get_name
...
def calc_accuracy(outputs, labels, loss_type, res):
    if loss_type == "multi_classification":
        if len(labels[0]) != len(outputs[0]):
            raise ValueError('Input dimensions of labels and outputs must match.')#检测数据维度

        outputs = outputs.data#用于获取张量的纯数据部分（即不包含梯度等额外信息的张量）
        labels = labels.data
        ...
        return res
        #多标签分类需要对每个类别单独处理，因为多个类别可能同时为1
        #outputs 是一个二维张量，形状为 [batch_size, num_classes]。
        #outputs.size(1) 表示第二维的大小，即类别的总数（nr_classes）
        ...
        return res
        #labels 是one-hot编码，torch.max 会返回值为1的位置索引
